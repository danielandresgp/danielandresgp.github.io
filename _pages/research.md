...
### Stable allocations
   ...
   <p align="justify"> More formally, assume that the centers are chosen according to a Poisson process $Z \subset \mathbb R^d$ of intensity $\lambda$. Assume that every center $\zeta \in Z$ desires for itself a territory of random volume $V \subset \mathbb R^d$, which is as close to $\zeta$ as possible. Then there is a unique allocation of points $x \in \mathbb R^d$ to the centers $Z$ that is stable in the sense of Gale—Shapley.</p>
